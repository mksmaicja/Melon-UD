...
{
    std::string CorePath = std::string(Core::BasePath) + "\\maicjaload\\Dependencies\\Bootstrap.dll";
    Core::Path = new char[CorePath.size() + 1];
    std::copy(CorePath.begin(), CorePath.end(), Core::Path);
    Core::Path[CorePath.size()] = '\0';

    std::string BaseAssemblyPath = std::string(Core::BasePath) + "\\maicjaload\\MelonLoader.dll";
    ...
    if (!Game::IsIl2Cpp)
        return true;
    std::string AssemblyGeneratorPath = std::string(Core::BasePath) + "\\maicjaload\\Dependencies\\Il2CppAssemblyGenerator\\Il2CppAssemblyGenerator.dll";
