...
{
	constexpr auto defaultpath = L"maicjaload\\Dependencies\\Bootstrap.dll";
...
{
	MessageBoxA(nullptr, (reason + " " + (should_kill ? "Preventing Startup" : "Continuing without MelonLoader") + "...").c_str(), "maicjaload", MB_ICONERROR | MB_OK);
	if (should_kill)
		terminate_process();
}
...
{
	MessageBoxW(nullptr, (reason + L" " + (should_kill ? L"Preventing Startup" : L"Continuing without MelonLoader") + L"...").c_str(), L"maicjaload", MB_ICONERROR | MB_OK);
	if (should_kill)
		terminate_process();
}
